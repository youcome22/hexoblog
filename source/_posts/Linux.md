...
Linux 内核最初只是由芬兰人林纳斯·托瓦兹（Linus Torvalds）在赫尔辛基大学上学时出于个人爱好而编写的。主要受到 Minix 和 Unix 思想的启发。
...
![](E:\youcome\Linux\images\linux内核及发行版本的关系.jpg)

目前市面上较知名的发行版有：Ubuntu、RedHat、CentOS、Debian、Fedora、SuSE、OpenSUSE、Arch Linux、SolusOS 等。

![](E:\youcome\Linux\images\目前市面上较知名的发行版本.jpg)
...
1.桥接模式，虚拟系统可以和外部系统通讯，但因处于同一网段会容易造成IP冲突

2.NAT模式，网络地址转换模式，虚拟系统外部通讯，不会造成IP冲突，但只能是虚拟系统与外部网络的单项通讯

3.主机模式，即是独立的系统，不参加与外部通讯
...
恢复到原先的某一状态，以防止可能有些误操作造成系统异常



**VMtools**

1.VMtools可以让我们再window下更好的管理vm虚拟机

2.可以设置window和Linux的共享文件夹
...
![](E:\youcome\Linux\images\内核引导示意图.png)
...
![](E:\youcome\Linux\images\运行init示意图.png)
...
![](E:\youcome\Linux\images\运行级别示意图.png)
...
![](E:\youcome\Linux\images\系统初始化示意图.png)
...
![](E:\youcome\Linux\images\用户登录系统示意图.png)

------

对于运行级别为5的图形方式用户，他们的登录是通过一个图形化的登录界面。登录成功后可以直接进入 KDE、Gnome 等窗口管理器。
...
默认我们登录的就是第一个窗口，也就是tty1，这个六个窗口分别为tty1,tty2 … tty6，你可以按下 `Ctrl + Alt + F1 ~ F6` 来切换。
...
![](E:\youcome\Linux\images\图形模式与文字模式的切换方式.png)
...
关机指令为：shutdown ，可以 `man shutdown` 来看一下帮助文档。
...
Linux的文件系统是采用级层式的树状目录结构，在此结构的最上层是根目录 **"/"**，在此目录下再创建其他的目录。

”在Linux中，一切皆文件“

![](E:\youcome\Linux\images\树状目录结构.jpg)
...
该工具可以生成三种格式的key ：SSH-1(RSA) SSH-2(RSA) SSH-2(DSA) ，我们采用默认的格式即 SSH-2(RSA)。Number of bits in a generated key 这个是指生成的key的大小，这个数值越大，生成的key就越复杂，安全性就越高。

至此，密钥对已经生成了。你可以给你的密钥输入一个密码，也可以留空。然后点 Save public key 保存公钥，点 Save private Key 保存私钥。
...
3）打开刚才生成的 public key 文件，复制从 AAAA开头至 "---- END SSH2 PUBLIC KEY ----" 该行上的所有内容，粘贴到 `/root/.ssh/authorized_keys`  文件中，要保证所有字符在一行。
...
![](E:\youcome\Linux\images\命令行权限示意图.jpg)

------

每个文件的属性由左边第一部分的 10 个字符来确定

![](E:\youcome\Linux\images\用户权限示意图.png)
...
所以等下我们设定权限的变更时，该文件的权限数字就是 **770**。变更权限的指令 chmod 的语法是这样的：
...
- 绝对路径：
  路径的写法，由根目录 **/** 写起，如`/usr/share/doc` 这个目录。
- 相对路径：
  路径的写法，不是由 **/** 写起，如由 `/usr/share/doc` 到 `/usr/share/man` 底下时，可以写成： **cd ../man** 这就是相对路径的写法。
...
如果使用 -m ，如上例给予 `-m 711` 来给予新的目录 drwx--x--x 的权限。
...
此命令创建了一个用户sam，其中 -d 和 -m 选项用来为登录名 sam 产生一个主目录 `/home/sam`（/home为默认的用户主目录所在的父目录）。
...
用户管理工作涉及的最重要的一个文件。Linux系统中的每个用户都在 `/etc/passwd` 文件中有一个对应的记录行，它记录了这个用户的一些基本属性。这个文件对所有用户都是可读的。
...
因此，现在许多Linux 系统（如SVR4）都使用了shadow技术，把真正的加密后的用户口令字存放到 `/etc/shadow` 文件中，而在 `/etc/passwd`文件的口令字段中只存放一个特殊的字符，例如“x”或者“*”。
...
**1、除上面列出的伪用户外，还有许多标准的伪用户，如：audit, cron, mail, usenet等，它们也都各自为相关的进程和文件所需要**
...
**2、`/etc/shadow` 中的记录行与 `/etc/passwd` 中的一一对应，它由pwconv命令根据 `/etc/passwd` 中的数据自动产生**
...
3. "最后一次修改时间"表示的是从某个时刻起，到用户最后一次修改口令时的天数。时间起点对不同的系统可能不一样。如在SCO Linux 中，这个时间起点是1970年1月1日。
...
vim 的官方网站显示vim 是一个程序开发工具而不是文字处理软件。

![](E:\youcome\Linux\images\vim 键盘图.gif)
...
此状态下敲击键盘动作会被Vim识别为命令，而非输入字符。此时按下 `i`，并不会输入一个字符，`i` 被当作了一个命令。
...
底线命令中输入：`set mouse = nv`  可鼠标定位光标

![](E:\youcome\Linux\images\vim工作模式.png)
...
| u                | 复原前一个动作。(常用)                                       |
| [Ctrl]+r         | 重做上一个动作。(常用)                                       |
| .                | 就是小数点！意思是重复前一个动作的意思。 如想要重复删除、重复贴上等等动作，按下小数点『.』就好了 (常用) |
...
| :----------------------- | ------------------------------------------------------------ |
| :w                       | 将编辑的数据写入硬盘档案中(常用)                             |
| :w!                      | 若文件属性为『只读』时，强制写入该档案。不过到底能不能写入， 还是跟对该档案的档案权限有关 |
| :q                       | 离开 vi (常用)                                               |
...
> 特别注意，在 vi/vim 中，数字是很有意义的！数字通常代表重复做几次的意思！ 也有可能是代表去到第几个等意思。
...
Linux系统的yum源默认是国外的链接，在国内可能会因为网络问题无法正常使用。
...
将yum设置为国内镜像可以提升软件包安装和更新的速度，同时避免一些常见软件版本无法找到。
...
除了网易之外，国内还有其他不错的 yum 源，比如中科大和搜狐等...
...
apt 命令提供了查找、安装、升级、删除某一个、一组甚至全部软件包的命令，命令简洁而又好记。
